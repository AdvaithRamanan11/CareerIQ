...
// Used for lifetime earnings projections (geometric series sum over 30 years).import { AREA_MULTIPLIERS, EXPERIENCE_MULTIPLIERS, REGION_AREA_MULTIPLIERS } from '../data/salaries.js';
import ratesData from '../data/rates.json' with { type: 'json' };
...
// ─── Salary Calculator ──────────────────────────────────────────────────────
// Resolve the area (urban/suburban/rural) multiplier for a given region.
// If a region is supplied AND the region×area grid has that cell, use the
// region-specific factor; otherwise fall back to the national factor.
// This makes `region` fully optional: callers that pass no region (or the
// "National" sentinel) get exactly the pre-feature national behavior.
export function resolveAreaMultiplier(area, region) {
  if (region && REGION_AREA_MULTIPLIERS[region] && REGION_AREA_MULTIPLIERS[region][area] != null) {
    return REGION_AREA_MULTIPLIERS[region][area];
  }
  return AREA_MULTIPLIERS[area] ?? 1.0;
}

export function calculateSalary({ college, job, area, experience, region }) {
  ...
  // Region-aware location adjustment. Region is optional; when absent or set to
  // the "National" sentinel, resolveAreaMultiplier returns the national factor,
  // so existing behavior is unchanged.
  const areaMult = resolveAreaMultiplier(area, region);
...
// ─── What-If Scenarios ──────────────────────────────────────────────────────
export function generateScenarios({ college, job, region }) {
  if (!college || !job) return [];
  const areas = ["Urban", "Suburban", "Rural"];
  const experiences = ["Entry", "Early Career", "Experienced", "Veteran"];
  const results = [];
  const entrySalary = calculateSalary({ college, job, area: "Suburban", experience: "Entry", region });
  for (const experience of experiences) {
    for (const area of areas) {
      const salary = calculateSalary({ college, job, area, experience, region });
...
export function compareSchools({ collegeA, collegeB, job, area, experience, region }) {
  if (!collegeA || !collegeB || !job) return null;
  const salaryA = calculateSalary({ college: collegeA, job, area, experience, region });
  const salaryB = calculateSalary({ college: collegeB, job, area, experience, region });
